pipeline {
    ...
    stages {
        ...
        stage('Load Image into kind') {
            steps {
                sh "kind load docker-image vertex-symposium:${env.BUILD_NUMBER} --name event-manager"
        ...
        stage('Deploy to Kubernetes') {
            steps {
                sh "sed -i 's/vertex-symposium:latest/vertex-symposium:${env.BUILD_NUMBER}/g' k8s/deployment.yaml"
